...
*   **Serialisierung:** Sie stellt sicher, dass Aufrufe an den `APSCore` nacheinander (sequenziell) erfolgen.
...
**Regel:** 
Nutze `atomic { ... }` im `APSCore` für alle Operationen, die:
...
*   **Ausgehend (Query/Observe):** Ergebnisse werden ausschließlich über `StateFlow` oder Eventhandler in Form von Callbacks nach außen gereicht.
...
3.  **Wird in der Pipeline suspendiert?** -> Gesamten Block in `APSCore` mit dem `atomicOperationLock` absichern. Siehe Hinweise unten.
